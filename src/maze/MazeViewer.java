...
public class MazeViewer extends JPanel {
	...
	/**
	 * Main driver function for the program. Calls various maze solving and maze
	 * formation algorithms, depending on the input flags.
	 *
	 * @param args
	 *            The arguments sent to the method via the console.
	 */
	public static void main(String args[]) {
		...
		// make the maze could be seen completely when initialization
		frame.setSize(viewer.getWidth() + 150, viewer.getHeight() + 150);
	...
	/**
	 * Initializes the MazeViewer by initializing the maze according to the
	 * input arguments. Checks that the arguments correspond to expected values.
	...
	/**
	 * Repaints the panel with the maze. Sets the current cell to the parameter;
	 * the current cell will be colored. Then waits, so the user can observe the
	 * visualization.
	 * 
	 * @param cell
	 *            The current cell, to be colored.
	 * @param pauseTime
	 ...
	 * drawn. Then if the cell is the "start" cell for exploring the maze, a
	 * green square is drawn within the cell; if it is the "end" cell, a red
	 * square is drawn within the cell; and if it is the "current" cell the
	 * algorithm is looking at, a blue square is drawn within the cell. Finally,
	 * all cells that have been "visited" are colored yellow.
	 *
	 * @param g
	 *            Graphics object on which to draw the maze.
	 */
	 ...
	 * 
	 * @param args
	 *            one of "dfs", "bfs" or "random"
	 */
	private boolean checkArgs(String[] args) {
		if (!(args[0].equals("dfs") || args[0].equals("bfs") || args[0].equals("random")))
			return false;
		return true;
	}
